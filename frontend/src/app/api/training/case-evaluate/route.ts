...
export async function POST(request: NextRequest) {
  ...
  return new Promise<NextResponse>((resolve) => {
    const req = http.request(
      { hostname: '127.0.0.1', port: 8000, path: '/api/training/case-evaluate', method: 'POST',
        agent: false,
    ...
    req.setTimeout(15_000, () => { req.destroy(); resolve(NextResponse.json({ error: 'timeout' }, { status: 504 })); });
